 ...
 */
public class MeetBookListViewActivity extends Activity implements OnScrollListener {

	private ListView goodsListView;
	private MeetBookListViewAdapter adapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		myApp =  (MyApp) MeetBookListViewActivity.this.getApplication();
		keyword = MeetBookListViewActivity.this.getIntent().getStringExtra("keyword");
		...
		adapter =  new MeetBookListViewAdapter(MeetBookListViewActivity.this);
...
//		loadingGoodsListData(null);
	...
	private void ShowSureDialog(final String roomid) {
		final AlertDialog alertDialog = new AlertDialog.Builder(MeetBookListViewActivity.this)
				.setTitle("确认信息")
				.setMessage("确认取消本次预定？")
...
//					JSONObject obj = new JSONObject(json);
					...
					if(arrlist.equals("1")){
						Toast.makeText(MeetBookListViewActivity.this, "取消成功", Toast.LENGTH_LONG).show();
						loadingGoodsListData();
					}
					}else{
					}
			...
			e.printStackTrace();
			result = "网络异常！";
			Toast.makeText(MeetBookListViewActivity.this, result, Toast.LENGTH_SHORT).show();;
		} catch (IOException e) {
			e.printStackTrace();
			result = "网络异常！";
			Toast.makeText(MeetBookListViewActivity.this, result, Toast.LENGTH_SHORT).show();;
	...
	public void loadingGoodsListData(){
		...
		String url = HttpUtil.URL_MEETBOOKLST+"userid="+myApp.getLoginKey();
		HttpPost request = HttpUtil.getHttpPost(url);
		String result = null;
		try {
			// 获得响应对象
