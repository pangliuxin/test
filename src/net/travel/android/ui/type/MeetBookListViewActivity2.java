...
import net.travel.android.adapter.MeetBookListViewAdapter2;
 ...
 */
public class MeetBookListViewActivity2 extends Activity implements OnScrollListener {

	private ListView goodsListView;
	private MeetBookListViewAdapter2 adapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.health_list_);
		myApp =  (MyApp) MeetBookListViewActivity2.this.getApplication();
		keyword = MeetBookListViewActivity2.this.getIntent().getStringExtra("keyword");
		...
		adapter =  new MeetBookListViewAdapter2(MeetBookListViewActivity2.this);
...
//		loadingGoodsListData(null);
	...
	private void ShowSureDialog(final String roomid) {
		final AlertDialog alertDialog = new AlertDialog.Builder(MeetBookListViewActivity2.this)
				.setTitle("确认信息")
				.setMessage("确认审核通过该预约信息？")
	...
	public void shenhe(String roomid){
		...
		String url = HttpUtil.URL_BOOK_COMFIRM+"meetbook.id="+roomid;
...
//					JSONObject obj = new JSONObject(json);
					...
					if(arrlist.equals("1")){
						Toast.makeText(MeetBookListViewActivity2.this, "操作成功", Toast.LENGTH_LONG).show();
						loadingGoodsListData();
					}
					}else{
					}
			...
			e.printStackTrace();
			result = "网络异常！";
			Toast.makeText(MeetBookListViewActivity2.this, result, Toast.LENGTH_SHORT).show();;
		} catch (IOException e) {
			e.printStackTrace();
			result = "网络异常！";
			Toast.makeText(MeetBookListViewActivity2.this, result, Toast.LENGTH_SHORT).show();;
	...
	public void loadingGoodsListData(){
		...
		String url = HttpUtil.URL_MEETBOOKLSTALL;
		HttpPost request = HttpUtil.getHttpPost(url);
		String result = null;
		try {
			// 获得响应对象
